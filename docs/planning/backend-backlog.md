...
- Staff and Manager have equal authority to mark a table `OCCUPIED` or
  `AVAILABLE`. An eligible table may submit a waiter-call only through a valid
  OTP-authenticated customer-table visit bound to its QR credential; occupancy
  and scan reminders remain POS operations. The public command creates or
  returns that table's one pending call and grants no order, payment, receipt,
  tracking, or catalog authority.
