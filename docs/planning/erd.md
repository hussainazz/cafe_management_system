...
erDiagram
    ...
    Product {
        ...
        string name
        integer priceAmount
        integer preparationDeadlineMinutes
        integer displayOrder
        boolean isActive
        boolean isAvailable
        datetime archivedAt
    ...
    Order {
        ...
        integer tableSeatingLimitSnapshotMinutes
        datetime estimatedTableReleaseAt
        uuid deletedById FK
        datetime deletedAt
    ...
    OrderItem {
        ...
        integer preparationDeadlineSnapshotMinutes
        integer quantity
        string note
        integer discountAmount
    ...
    CafeSettings {
        uuid id PK
        integer defaultTableSeatingLimitMinutes
        uuid updatedById FK
        datetime updatedAt
    }
...
| `...Id FK`       | **Foreign key**. A stored link to another table's `id`, such as `PaymentSettlement.orderId` linking a settlement to the order it settles. The exact referential rules are listed in `database-constraints.md`. |
...
specified in `database-constraints.md` rather than implied only by the diagram.
...
| `preparationDeadlineMinutes` | Current product preparation deadline in whole minutes. Staff POS, QR menu, and order ETA calculations use it for current catalog reads. |
...
| `estimatedPreparationMinutes`      | Maximum preparation-deadline snapshot across the order items. It is recalculated by the server when an unpaid order's content changes.                                                       |
| `tableSeatingLimitSnapshotMinutes` | Effective seating limit captured when a table order is created. It is empty for takeaway orders.                                                                                             |
| `estimatedTableReleaseAt`          | Server-derived UTC timestamp for when the table is expected to be free: order creation time plus seating-limit snapshot plus estimated preparation minutes. It is empty for takeaway orders. |
...
| `preparationDeadlineSnapshotMinutes` | Product preparation deadline copied at sale time in whole minutes. A later catalog timing change must not alter historical table estimates. |
...
| `reference`    | Optional reference for `CARD_TRANSFER`; empty for `CARD_TERMINAL` and cash.                                     |
...
| `defaultTableSeatingLimitMinutes` | Default seating-limit minutes applied when new tables are created unless the Manager sets a table-specific value. The initial value is 45. |
...
- `Order` is created by a logged-in user with state `OPEN` and payment status `UNPAID`. Active settlement allocations determine `UNPAID`, `PARTIALLY_PAID`, or `PAID`; only the lifecycle state transitions to `DELETED`. Additional items may be added while the order remains `OPEN`, including after partial or full settlement; those additions create or extend only unsettled item quantities and never rewrite posted settlement allocations, tenders, or receipt history. `DELETED` is logical deletion, so the order, items, settlements, tenders, reversals, timing snapshots, and audit trail remain available.
