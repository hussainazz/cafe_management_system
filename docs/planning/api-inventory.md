...
| `PATCH` | `/api/v1/orders/:orderId`                                   | Staff  | 2     | Apply a controlled edit to an `OPEN` order; requires the current order version. After any settlement, edits may add items, increase quantities, or adjust only unsettled item quantities and notes; settled quantities, posted allocations, tenders, and receipts are immutable.                      |
...
| Method | Path                                 | Access | Stage | Purpose                                                                                                                                                                                |
| ------ | ------------------------------------ | ------ | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/api/v1/public/menu`                | Public | 3     | Read the complete browse-only menu, including active categories, products, options, availability, product preparation-deadline minutes, each product's single image, and final prices. |
| `GET`  | `/api/v1/public/products/:productId` | Public | 3     | Read one current, publicly visible product and its preparation-deadline minutes plus available option groups/options.                                                                  |
...
| `PATCH` | `/api/v1/admin/products/:productId`                                    | Manager | 6     | Update product details, final price, preparation-deadline minutes, active state, availability, display order, or allowed option groups.           |
