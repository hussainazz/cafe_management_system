...
| Customers      | Customers browse the generic menu anonymously. A table-scoped QR credential asks for a phone number on first use and creates a short table visit that authorizes only a waiter-call; OTP verification is intentionally deferred to a later phase. When Staff or Manager explicitly holds an eligible table occupied, that table alone may temporarily bypass customer phone collection for its current QR context; returning it to available clears the bypass. Collected phones are unverified and must not be used for marketing until OTP/consent work is added. There are no order, payment, tracking, loyalty, or marketing capabilities. |
...
- Customer self-ordering, customer carts submitted to the café, table-order authority, guest order tracking, loyalty, wallets, coupons, and marketing automation. The phone-backed customer identity exists only to authorize a table-scoped waiter call and is not order or payment authority; phone verification and marketing consent are deferred.
...
- Native mobile apps, advanced forecasting, multi-printer routing beyond the locally selected Windows queue, and a CDN requirement.
...
| --------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPEN`    | Staff has registered the order. Its payment status may be `UNPAID` or `PARTIALLY_PAID`.                              | Update the same order in place by adding items, increasing quantities, adjusting only unsettled quantities, transferring table, or settling selected unallocated item quantities. As a separate action, Staff may mark the order `DELETED`. Settled quantities and posted settlement records are immutable. |
| `CLOSED`  | A final settlement fully paid the order. Table orders have atomically freed their table and invalidated its prior guest context; takeaway orders leave active POS work. | No normal content, table, or payment edits. History, receipts, settlements, and audit records remain available; an audited Manager settlement reversal reopens the order and, for a table order, marks its table occupied again without restoring the invalidated guest context. |
...
- A settlement selects one or more unallocated order-item quantities or an integer Toman amount up to the remaining balance. The server calculates quantity-based amounts from immutable item snapshots and deterministic discount allocation; amount-based payments are deterministically allocated across remaining item snapshots in display order. An item cannot be over-allocated across active settlements.
...
- `UNPAID`, `PARTIALLY_PAID`, and `PAID` are derived from active settlement allocations and persisted on the order for efficient POS reads. When a settlement makes an order `PAID`, the server atomically transitions it to `CLOSED`; for a table order it also marks the table `AVAILABLE`, clears occupancy metadata, invalidates the prior guest context, and resolves any pending waiter call. Overpayment and an unallocated tender balance are forbidden.
...
- Order-create diagnostics correlate the POS-generated request ID with the authenticated actor, channel, requested table UUID, client-displayed table name, and the server-resolved table UUID/name. These fields are operational diagnostics only: the server-resolved table remains authoritative, and diagnostics never log cookies, QR tokens, idempotency keys, item notes, payment references, or raw request bodies.
...
- A Manager may enable one café-wide seating-limit minutes value in café settings. When it is not enabled, its value is `null` and POS table timing is not shown or required. When enabled, the positive whole-minute value is stored in configuration/data rather than embedded in POS UI code.
- Active POS table timing reads the current enabled café seating limit. Disabling or changing it affects only the display; it never rewrites historical order/payment data.
- Active catalog option groups are canonical and uniquely named. Their option catalog is defined once; each product explicitly exposes an allowed subset, optional per-product price overrides, and a selection range. Historical order-item option snapshots are immutable even after a group or option is archived.
...
- Every newly created table or takeaway order receives the next positive integer in one shared `Asia/Tehran` business-day sequence. The sequence restarts at `1` on the next Tehran day, is assigned atomically in the order transaction, and an idempotent replay never consumes another number. UUID remains the durable order identity.
