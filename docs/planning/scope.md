...
| Customers      | Customers browse the generic menu anonymously. A table-scoped QR credential normally requires full-name/phone OTP verification to create a short table visit that authorizes only a waiter-call. When Staff or Manager explicitly holds an eligible table occupied, that table alone may temporarily bypass OTP for its current QR context; returning it to available clears the bypass. There are no order, payment, tracking, loyalty, or marketing capabilities. |
...
- Customer self-ordering, customer carts submitted to the café, table-order authority, guest order tracking, loyalty, wallets, coupons, and marketing automation. The OTP-backed customer identity exists only to authorize a table-scoped waiter call and is not order or payment authority.
