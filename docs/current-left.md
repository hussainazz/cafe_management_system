...
- 21 September 2026 weighted per-kilogram pricing: production now has the reviewed migration and the hidden `GROUND_COFFEE` weighted product with its required coffee-line choices. Its initial base price is intentionally `0` Toman for Manager configuration; existing coffee-line option deltas remain `0`, `20,000`, and `60,000` Toman. Existing quantity-based settlement semantics remain discrete bag/item counts. Left: Manager pricing confirmation and browser/device POS acceptance.
