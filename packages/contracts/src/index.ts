...
const ReceiptPaymentSchema = z.object({ method: z.enum(["CASH", "CARD_TERMINAL", "CARD_TRANSFER"]), amount: z.number().int().positive() });
...
export const OrderReceiptResponseSchema = z.object({ data: z.object({ displayTime: z.string(), items: z.array(ReceiptItemSchema), subtotalAmount: z.number().int().nonnegative(), discountAmount: z.number().int().nonnegative(), totalAmount: z.number().int().nonnegative(), paidAmount: z.number().int().nonnegative(), balanceAmount: z.number().int().nonnegative(), payments: z.array(ReceiptPaymentSchema) }), meta: z.object({ requestId: z.string() }) });
export const SettlementReceiptResponseSchema = z.object({ data: z.object({ displayTime: z.string(), items: z.array(ReceiptItemSchema), totalAmount: z.number().int().nonnegative(), payments: z.array(ReceiptPaymentSchema) }), meta: z.object({ requestId: z.string() }) });
