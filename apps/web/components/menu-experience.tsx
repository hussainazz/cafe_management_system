...
export function MenuExperience({ initialMenu, initialRequestFailed, invalidTableContext }: MenuExperienceProps) {
  ...
  async function requestOtp() {
    ...
    try {
      const response = await fetch("/api/customer-otp/request", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ phoneNumber }) });
  ...
  if (tableContext?.active && tableContext.authenticationRequired) {
    return (
      <main className="error-page" dir="rtl">
        ...
        <h1>برای استفاده از میز، شماره موبایل خود را تأیید کنید</h1>
        ...
        {otpChallengeId ? (
          ...
          <form onSubmit={(event) => { event.preventDefault(); void requestOtp(); }}>
            ...
            <button type="submit" disabled={otpLoading}>{otpLoading ? "در حال ارسال…" : "ارسال کد تأیید"}</button>
          </form>
        )}
