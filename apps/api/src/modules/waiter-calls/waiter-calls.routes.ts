...
export const waiterCallRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    ...
    { schema: { tags: ["Public customer authentication"], body: zodToJsonSchema(CustomerOtpVerifySchema), response: { 200: zodToJsonSchema(CustomerAuthStateResponseSchema), 400: errorResponse, 401: errorResponse, 409: errorResponse, 429: errorResponse } } },
    async (request, reply) => {
      const context = await credentialFromCookie(app.prisma, request.headers.cookie);
      if (!context) throw new ApplicationError(401, ErrorCodes.TABLE_CONTEXT_INVALID, "Table context required.");
  ...
  app.get(
    ...
    { schema: { tags: ["Public customer authentication"], response: { 200: zodToJsonSchema(CustomerAuthStateResponseSchema) } } },
    async (request) => {
      const auth = await readCustomerAuth(app.prisma, request.headers.cookie);
      const context = await credentialFromCookie(app.prisma, request.headers.cookie);
