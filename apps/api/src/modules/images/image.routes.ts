...
export const imageRoutes: FastifyPluginAsync = async (app) => {
  app.put("/admin/products/:productId/image", { preHandler: requireManagerRoute }, async (request: any) => {
    ...
    try {
      const image = await app.prisma.$transaction(async (tx) => {
        const updated = await tx.productImage.upsert({ where: { productId }, create: { productId, storageKey, altText }, update: { storageKey, altText } });
        await tx.auditLog.create({ data: { actorId: request.authenticatedUser!.id, requestId: request.id, operation: "UPSERT_PRODUCT_IMAGE", entityType: "PRODUCT", entityId: productId, afterSnapshot: updated } });
        return updated;
      });
      committed = true;
  ...
  app.patch("/admin/products/:productId/image", { preHandler: requireManagerRoute }, async (request: any) => { const altText = typeof request.body?.altText === "string" ? request.body.altText.trim() : ""; if (!altText || altText.length > 500) throw new ApplicationError(400, ErrorCodes.VALIDATION_ERROR, "Alt text is invalid."); const image = await app.prisma.$transaction(async (tx) => { const updated = await tx.productImage.update({ where: { productId: request.params.productId }, data: { altText } }); await tx.auditLog.create({ data: { actorId: request.authenticatedUser!.id, requestId: request.id, operation: "UPDATE_PRODUCT_IMAGE", entityType: "PRODUCT", entityId: request.params.productId, afterSnapshot: updated } }); return updated; }); return { data: { storageKey: image.storageKey, altText: image.altText }, meta: { requestId: request.id } }; });
  app.post("/admin/products/:productId/image/archive", { preHandler: requireManagerRoute }, async (request: any) => { const image = await app.prisma.$transaction(async (tx) => { const deleted = await tx.productImage.delete({ where: { productId: request.params.productId } }); await tx.auditLog.create({ data: { actorId: request.authenticatedUser!.id, requestId: request.id, operation: "ARCHIVE_PRODUCT_IMAGE", entityType: "PRODUCT", entityId: request.params.productId, afterSnapshot: { storageKey: deleted.storageKey } } }); return deleted; }); await unlink(imagePath(image.storageKey)).catch(() => undefined); return { data: { productId: request.params.productId }, meta: { requestId: request.id } }; });
