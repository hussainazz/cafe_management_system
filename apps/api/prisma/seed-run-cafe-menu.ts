...
// Snapshot of https://runcafe.payanitamenu.com/ on 21 August 2026.
// Prices are stored in Toman, so the public site's "thousand Toman" values are multiplied by 1,000.
const menu = `
...
async function main() {
  for (const [categoryOrder, line] of menu.split("\n").entries()) {
    ...
    const products = line.slice(categorySeparator + 1);
    const category = await prisma.category.findFirst({ where: { name, archivedAt: null }, orderBy: { displayOrder: "asc" } })
      ?? await prisma.category.create({ data: { name, displayOrder: categoryOrder + 1 } });
    for (const [productOrder, rawProduct] of products.split(",").entries()) {
      ...
      const existing = await prisma.product.findFirst({ where: { categoryId: category.id, name: productName, archivedAt: null } });
      if (!existing) await prisma.product.create({ data: { categoryId: category.id, name: productName, priceAmount, preparationDeadlineMinutes: 10, displayOrder: productOrder + 1 } });
    }
  }
}
